...
template <typename NodeT>
void DependencyNode<NodeT>::addDependencyFrom(DependencyNode<NodeT> *pNode, DependencyType type)
{
  if (pNode == this)
    return;
  DependencyLink link = DependencyLink(pNode, type);
  if (std::find(reverseDepList.begin(), reverseDepList.end(), link) == reverseDepList.end())
  {
    reverseDepList.push_back(link);
  }
}
