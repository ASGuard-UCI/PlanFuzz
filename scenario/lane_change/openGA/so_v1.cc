...
bool eval_solution(const MySolution& p, MyMiddleCost &c){
  ...
  double fit1 = get_fitness_shared_mem();
  double fit2 = 0.0;
  //double fit2 = barrier_scale * (mutator.BarrierFunc(&tmp_msg) );
  c.fitness = fit1+ fit2;
  //c.fitness = get_fitness_shared_mem() +  barrier_scale* mutator.BarrierFunc(&tmp_msg);
  ...
  msg_out.close();
  std::cout<<ss.str()<<'\n';
  if (mutator.BarrierFunc(&tmp_msg) + mutator.OnLaneBarrierFunc(&tmp_msg) != std::numeric_limits<double>::infinity()){
  ...
  output_file.open("log_fit.txt", std::ios::in|std::ios::app);
  output_file<<case_index<<" "<<fit1<<" "<<fit2<<'\n';
  output_file.close();
  case_index++;
  if (case_index%100 == 0 ) 
    barrier_scale = barrier_scale/2.0;
  std::cout<<"Before end eval func\n";
  return true;
}
...
MySolution mutate(const MySolution& X_base, const std::function<double(void)> &rnd01, double shrink_scale){
  ...
  mutator.MutatePlanningMsgTrans(&X_ret.input);
  return X_ret;
}
...
int main(int argc, char** argv){
  ...
  mutator.LoadBlackList("/apollo/modules/tools/fuzz/vuln_test/scenario/lane_change/lane_change_blacklist");
  ...
  //output_file.close();
  std::cout<<"Before stop the fuzzer\n";
  fuzzer.Stop();
  return 0;
}
