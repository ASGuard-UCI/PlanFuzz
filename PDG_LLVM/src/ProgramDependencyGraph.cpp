...
bool pdg::ProgramDependencyGraph::runOnModule(Module &M)
{
  ...
  errs()<<"Start the PDG pass!\n";
  module = &M;
  inst_stack = new pdg::InstructionStack();
  ...
  for (Module::iterator FI = M.begin(); FI != M.end(); ++FI)
  {
    ...
    if (isFiltered(Func))
      continue;
    errs()<<"After filtering"<<Func->getName()<<'\n';
    ...
    for (inst_iterator I = inst_begin(Func); I!=inst_end(Func); I++){
	  ...
	  continue;
	if (isFiltered(c->getCalledFunction()))
	  continue;
	if (calledGraph.find(c->getCalledFunction())==calledGraph.end())
	  calledGraph[c->getCalledFunction()] = std::set<Function*>();
	calledGraph[c->getCalledFunction()].insert(Func);
      }
    }
  }
  ...
  errs()<<"CHeck point 1\n";
  std::set<std::string> invoked_func_list;
  std::set<llvm::Instruction*> target_set;
  std::stack<llvm::Instruction*> expand_stack;
  for (Module::iterator FI = M.begin(); FI!=M.end(); ++FI){
    target_set.clear();
    for (inst_iterator I = inst_begin(*FI); I != inst_end(*FI); ++I){
      //errs()<<(*I)<<'\n';
      if (auto c = dyn_cast<CallInst>(&*I)){
	if (c->getCalledFunction()==NULL)
          continue;
        auto funcname = c->getCalledFunction()->getName();
        //errs()<<funcname.data()<<'\n';
        if (funcname.find("mark")!=-1
	     && funcname.find("targeted")!=-1 
	     && funcname.find("pos")!=-1){
          errs()<<"Find a target pos\n";
	  //target_set.insert(&*I);
	  expand_stack.push(&*I);
	  inst_stack->addInst(&*I);
	  inst_stack->UpdateDist(&*I, 0);
          inst_stack->UpdateIsCritical(&*I, true);
          inst_stack->UpdateCloserBranch(&*I, true);
        }

        //growInsideFunc(&*I, M);
        //auto dataDList = ddg->getNodeDepList(&*I);
        //for (auto dependencyPair : dataDList){
         // InstructionWrapper *DNodeW2 = const_cast<InstructionWrapper *>(dependencyPair.first->getData());
          //errs()<<"Dep: "<<*(DNodeW2->getInstruction())<<'\n';
          //errs()<<"Dep_type: "<<dependencyPair.second<<'\n';
        //}
  ...
  expandDependency(&expand_stack, M);
  for (Module::iterator FI = M.begin(); FI != M.end(); ++FI)
  {
    Function *Func = dyn_cast<Function>(FI);
    if (FI->isDeclaration())
...
void pdg::ProgramDependencyGraph::expandRetInst(std::stack<Instruction*> *expand_stack, Function* Func, int dist, Module &M){
  for (llvm::inst_iterator I = inst_begin(Func); I!=inst_end(Func); I++){
    if (auto c = dyn_cast<ReturnInst>(&*I)){
      if (!inst_stack->isInStack(&*I)){
        errs()<<"Push returnInst: "<<*I<<'\n';
        printDebugInfo(*I);
        inst_stack->addInst(&*I);
        expand_stack->push(&*I); 
        inst_stack->UpdateDist(&*I, dist);
        inst_stack->UpdateIsCritical(&*I, false);
      }
    }
  } 
}
...
void pdg::ProgramDependencyGraph::expandCallInst(std::stack<Instruction*> *expand_stack, Function* Func, Module &M){
  if (calledGraph.find(Func)!=calledGraph.end()){
    std::set<Function*> callee_set = calledGraph[Func];
    for (std::set<Function*>::iterator callee = callee_set.begin(); callee!=callee_set.end(); callee++){
      for  (llvm::inst_iterator I = inst_begin(*callee); I!=inst_end(*callee); I++){
        if (auto c = dyn_cast<CallInst>(&*I))
	  if (c->getCalledFunction()==Func){
            if (!inst_stack->isInStack(&*I)){
              errs()<<"Push callInst: "<<*I<<'\n';
	      printDebugInfo(*I);
	      inst_stack->addInst(&*I);
	      expand_stack->push(&*I);
	    }
...
int pdg::ProgramDependencyGraph::BBDistance(Instruction* prev, Instruction* next, Module &M){
  llvm::BasicBlock *prevB = prev->getParent();
  llvm::BasicBlock *nextB = next->getParent();
  if (prevB == nextB){
    errs()<<"Start BB "<<*prevB;
    errs()<<"End BB "<<*nextB;
    return 0;
  }
  std::queue<BasicBlock*> search_queue;
  std::queue<int> dist_queue;
  std::set<BasicBlock*> visited;
  search_queue.push(nextB);
  dist_queue.push(0);
  bool terminate = false;
  //errs()<<"\n--------------------------Start debug BBDist------------------------\n";
  //errs()<<" The start BB is "<<*prevB<<'\n';
  //errs()<<" The target BB is "<<*nextB<<'\n';
  while (!terminate){
    BasicBlock* curr = search_queue.front();
    search_queue.pop();
    double curr_dist = dist_queue.front();
    dist_queue.pop();
    //errs()<<"pop a BB "<<*curr<<'\n';
    for (BasicBlock *pred : predecessors(curr))
    {
      //errs()<<"a new branch"<<*pred<<'\n';
      if (pred == prevB)
	return curr_dist + 1;
      if (visited.find(pred)!=visited.end())
        continue;
      
      search_queue.push(pred);
      visited.insert(pred);
      dist_queue.push(curr_dist + 1);
    }
...
void pdg::ProgramDependencyGraph::expandDependency(std::stack<Instruction*> *expand_stack, Module &M){

  while (!expand_stack->empty()){
    Instruction* top = expand_stack->top();
    expand_stack->pop();
    errs()<<"Pop an inst: "<<(*top)<<'\n';
    printDebugInfo(*top);
    Function* Func = top->getFunction();
    errs()<<"Func in expand function: "<<Func<<" "<<Func->getName()<<'\n';
    if (ddg_map.find(Func)==ddg_map.end())
      ddg_map[Func] = &getAnalysis<DataDependencyGraph>(*Func);
    if (cdg_map.find(Func)==cdg_map.end())  
      cdg_map[Func] = &getAnalysis<ControlDependencyGraph>(*Func);
    ddg = ddg_map[Func];
    cdg = cdg_map[Func];
    if (cdg ==NULL)
      errs()<<"CDG is NULL\n";
    //Instruction is a callInst or InvokeInst
    if (auto c = dyn_cast<InvokeInst>(top))
      if (c->getCalledFunction()!=NULL){
	if (visited.find(c->getCalledFunction())==visited.end())
	  visited[c->getCalledFunction()] = false;
        expandRetInst(expand_stack, c->getCalledFunction(), inst_stack->QueryDist(top), M);
//	continue;
      }
    if (auto c = dyn_cast<CallInst>(top))
      if (c->getCalledFunction()!=NULL){
	if (visited.find(c->getCalledFunction())==visited.end())
	  visited[c->getCalledFunction()] = false;
        expandRetInst(expand_stack, c->getCalledFunction(), inst_stack->QueryDist(top), M);
    ...
    auto dataDList = cdg->getNodeReverseDepList(top);
    for (auto dependencyPair : dataDList){
      InstructionWrapper *DnodeW2 = const_cast<InstructionWrapper *>(dependencyPair.first->getData());
      if (DnodeW2 == NULL || DnodeW2->getInstruction()== NULL)
        continue;
      if (!pdg::CDGPrune(*(DnodeW2->getInstruction()))){
        if (!inst_stack->isInStack(DnodeW2->getInstruction())){
          expand_stack->push(DnodeW2->getInstruction());
          errs()<<"Push cdg: "<<*(DnodeW2->getInstruction())<<'\n';
	  printDebugInfo(*(DnodeW2->getInstruction()));
          inst_stack->addInst(DnodeW2->getInstruction());
	  inst_stack->UpdateDist(DnodeW2->getInstruction(), 
			  inst_stack->QueryDist(top) + BBDistance(DnodeW2->getInstruction(), top, M));
          inst_stack->UpdateIsCritical(DnodeW2->getInstruction(), 
                          inst_stack->QueryIsCritical(top));
          if (inst_stack->QueryIsCritical(top)){
            if (llvm::BranchInst::classof(DnodeW2->getInstruction())){
              llvm::BranchInst *br_inst;
              br_inst = static_cast<llvm::BranchInst*>(DnodeW2->getInstruction());
              if ((br_inst->isConditional())&&(br_inst->getNumSuccessors()==2)){
                int d_true = BBDistance(br_inst->getSuccessor(0)->getTerminator(), top, M);
                int d_false = BBDistance(br_inst->getSuccessor(1)->getTerminator(), top, M);
                inst_stack->UpdateTDist(DnodeW2->getInstruction(), 
                        d_true + inst_stack->QueryDist(top));
                 inst_stack->UpdateFDist(DnodeW2->getInstruction(), 
                        d_false + inst_stack->QueryDist(top));
                
                if (d_true < d_false)
                  inst_stack->UpdateCloserBranch(DnodeW2->getInstruction(), true);
                else
                  inst_stack->UpdateCloserBranch(DnodeW2->getInstruction(), false);
              }
            }
          }
        }
    ...
    auto dataCList = ddg->getNodeReverseDepList(top);
    for (auto dependencyPair : dataCList){
      InstructionWrapper *DnodeW2 = const_cast<InstructionWrapper *>(dependencyPair.first->getData());
      if (DnodeW2 == NULL || DnodeW2->getInstruction()==NULL)
        continue;
      if (!(pdg::DDGPrune(*(DnodeW2->getInstruction())))){
        if (!inst_stack->isInStack(DnodeW2->getInstruction())){
          expand_stack->push(DnodeW2->getInstruction());
          errs()<<"Push ddg: "<<*(DnodeW2->getInstruction())<<'\n';
	  printDebugInfo(*(DnodeW2->getInstruction()));
          inst_stack->addInst(DnodeW2->getInstruction());
	  inst_stack->UpdateDist(DnodeW2->getInstruction(), 
			  inst_stack->QueryDist(top));
          inst_stack->UpdateIsCritical(DnodeW2->getInstruction(), false);
        }
      }
    }
    //Entry point of Func
    if (visited.find(Func)==visited.end()){
      visited[Func] = true;
      errs()<<"Start expanding interprocedural callinst\n";
      expandCallInst(expand_stack ,Func, M);
    }
  }
  inst_stack->printList();
  inst_stack->printBrList();
}


void pdg::ProgramDependencyGraph::growInsideFunc(std::set<llvm::Instruction*> &inst_set, Module &M, std::set<std::string> &invoked_func_list){
  if (inst_set.size()==0)
    return;
  Function* Func = (*(inst_set.begin()))->getFunction();
  auto &pdgUtils = PDGUtils::getInstance();
  pdgUtils.constructFuncMap(M);
  pdgUtils.collectGlobalInsts(M);
  pdgUtils.categorizeInstInFunc(*Func);
  ...
  std::stack<Instruction*> temp_stack;
  for (std::set<llvm::Instruction*>::iterator it = inst_set.begin(); it!=inst_set.end(); ++it){
    temp_stack.push(*it);
  }

  while (!temp_stack.empty()){
    Instruction* top = temp_stack.top();
    temp_stack.pop();
    errs()<<"Pop an inst: "<<(*top)<<'\n';
    printDebugInfo(*top);
    if (auto c = dyn_cast<InvokeInst>(top)){
      if (c->getCalledFunction()!=NULL)
        invoked_func_list.insert(c->getCalledFunction()->getName().str());
    }
    if (auto c = dyn_cast<CallInst>(top)){
      if (c->getCalledFunction()!=NULL)
        invoked_func_list.insert(c->getCalledFunction()->getName().str());
    ...
    for (auto dependencyPair : dataDList){
      ...
      if (!pdg::CDGPrune(*(DnodeW2->getInstruction()))){
        if (!inst_stack->isInStack(DnodeW2->getInstruction())){
          temp_stack.push(DnodeW2->getInstruction());
          errs()<<"Push cdg: "<<*(DnodeW2->getInstruction())<<'\n';
	  printDebugInfo(*(DnodeW2->getInstruction()));
          inst_stack->addInst(DnodeW2->getInstruction());
	  inst_stack->UpdateDist(DnodeW2->getInstruction(), inst_stack->QueryDist(top)+1);
        }
      }
    }
    auto dataCList = ddg->getNodeReverseDepList(top);
    for (auto dependencyPair : dataCList){
	  ...
	  printDebugInfo(*(DnodeW2->getInstruction()));
          inst_stack->addInst(DnodeW2->getInstruction());
	  inst_stack->UpdateDist(DnodeW2->getInstruction(), inst_stack->QueryDist(top));
        }
      }
    }
  }
...
typename pdg::DependencyNode<pdg::InstructionWrapper>::DependencyLinkList pdg::ProgramDependencyGraph::getNodeReverseDepList(Instruction *inst)
{
  return PDG->getNodeReverseDepList(PDGUtils::getInstance().getInstMap()[inst]);
}
