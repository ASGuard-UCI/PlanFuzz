...
template <typename NodeT>
typename DependencyNode<NodeT>::DependencyLinkList DependencyGraph<NodeT>::getNodeReverseDepList(const NodeT *data) 
{
  DependencyNode<NodeT>* dNode = getNodeByData(data);
  return dNode->getReverseDepList();
}
...
void DependencyGraph<NodeT>::addDependency(const NodeT *from, const NodeT *to, DependencyType depType)
{
  ...
  toNode->addDependencyFrom(fromNode, depType);
}
